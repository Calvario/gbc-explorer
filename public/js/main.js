var getAfterId = undefined;
var currentTab = undefined;
var currentBlock = undefined;
var allItemInfo = undefined;
...
function homeBlocks(afterId) {
  const pagination = afterId === undefined ? '' : '?limit=100&afterId=' + afterId;
  $.get('/rest/api/1/block' + pagination, function (data, textStatus, jqXHR) {
...
function homeTransactions(afterId) {
  const pagination = afterId === undefined ? '' : '?limit=100&afterId=' + afterId;
  $.get('/rest/api/1/transaction' + pagination, function (data, textStatus, jqXHR) {
    if (afterId === undefined) {
      // Clean table
...
function addressTransactions(afterId) {
  const pagination = afterId === undefined ? '' : '?limit=100&afterId=' + afterId;
  $.get('/rest/api/1/address/' + getUrlParameter() + '/transactions' + pagination, function (data, textStatus, jqXHR) {
    if (afterId === undefined) {
      // Clean table
...
function extractionBlocks(afterId) {
  const pagination = afterId === undefined ? '' : '?limit=100&afterId=' + afterId;
  $.get('/rest/api/1/extraction/' + getUrlParameter() + pagination, function (data, textStatus, jqXHR) {
    if (afterId === undefined) {
      // Clean table
...
$(document).ready(function() {
  ...
  if ($(location).attr('pathname') === '/') {
    ...
    $('#homeBlocksGetMore').click(function(){
      homeBlocks(getAfterId);
      // Avoid auto refresh
    ...
    $('#homeTransactionsGetMore').click(function(){
      homeTransactions(getAfterId);
      // Avoid auto refresh
      currentTab = undefined;
    });
  ...
  if ($(location).attr('pathname').indexOf('/extraction') === 0) {
    ...
    $('#extractionGetMore').click(function(){
